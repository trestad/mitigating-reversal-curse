...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    #     """
        ...
        # q_len = labels.shape[-1]
        # for idx, row in enumerate(labels):
        #     start_pos = random.randint(7, q_len - 6) 
        #     # 将选择的位置及其后续的5个位置置零
        #     row[start_pos:start_pos + 3] = -100
        #     inputs[idx][start_pos:start_pos + 3] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
        ...
        # The rest of the time (10% of the time) we keep the masked input tokens unchanged
        # return inputs, labels #, torch.cat([masked_indices[:,1:].long(),torch.zeros(labels.shape[0],1)],dim=-1)
    def torch_mask_tokens(self, inputs: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        ...
        if special_tokens_mask is None:
            ...
            special_tokens_mask = special_tokens_mask.bool()
        raise ValueError("Test do not pass")
